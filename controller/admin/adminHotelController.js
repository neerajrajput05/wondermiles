...
const add = async (req, res, next) => {
    try {
        ...
        if (! fetch_admin) 
            return res.status(404).status(404).json({status: false, msg: 'Admin not exists'})
        
        const fetch_category = await adminCategoryModel.find({_id: categoryId})
        if (! fetch_category) 
            return res.status(404).status(404).json({status: false, msg: 'Category not exists'})
        
        if (!title) 
            return res.status(404).json({status: false, msg: 'Please provide the title.'});
        
        if (!subTitle) 
            return res.status(404).json({status: false, msg: 'Please provide the sub title.'});
        
        if (!type) 
            return res.status(404).json({status: false, msg: 'Please provide the type.'});
        
        if (!destination) 
            return res.status(404).json({status: false, msg: 'Please provide the destination.'});
        
        if (!description) 
            return res.json({status: false, msg: 'Please provide the description'});
        
        if (!logo) 
            return res.json({status: false, msg: 'Please provide the logo'});
        
        if (!video) 
            return res.json({status: false, msg: 'Please provide the video'});
        
        if (!aminities) 
            return res.json({status: false, msg: 'Please provide the aminities'});
        
        if (!rules) 
            return res.json({status: false, msg: 'Please provide the rules'});
        
        if (!location) 
            return res.json({status: false, msg: 'Please provide the location'});
        
        if (!callUs) 
            return res.json({status: false, msg: 'Please provide the callUs'});
        
        if (!code) 
            return res.json({status: false, msg: 'Please provide the code'});
        ...
        addHotel.save((err) => {
            ...
            if (err) {
                ...
                var dataTosent = [];
                aminities.forEach(async (e, index) => { // console.log('e', e, 'index', index)
                    dataTosent.push({
                        hotelId: addHotel._id,
                        aminitiesId: e,
                        type: 'aminities',
                        createdById: _id,
                        createdAt: timeStamp,
...
const hotelList = async (req, res, next) => {
    try {
        ...
        if (! fetch_hotel) 
            return res.status(404).status(404).json({status: false, msg: 'hotel not exists'})
...
const HotelPreview = async (req, res, next) => {
    try {
        ...
        if (! fetch_hotel) 
            return res.status(404).status(404).json({status: false, msg: 'The hotel not exists'})
        
        const fetch_type = await adminCategoryModel.findOne({_id: fetch_hotel.type, type: 'sub_main'})
        if (! fetch_type) 
            return res.status(404).status(404).json({status: false, msg: 'The hotel type not exists'})
        const fetch_destination = await adminDestinationModel.findOne({_id: fetch_hotel.destinationId, type: 'city'})
        if (! fetch_destination) 
            return res.status(404).status(404).json({status: false, msg: 'The hotel destination not exists'})
        ...
        if (! fetch_logo) 
            return res.status(404).status(404).json({status: false, msg: 'The hotel logo not exists'})
        
        return res.status(200).json({
            status: true,
            msg: 'successfully getting',
            data: fetch_hotel,
            aminities: fetch_hotelAminities,
            rules: JSON.parse(fetch_hotel.rules),
            callus: JSON.parse(fetch_hotel.callUs),
            hotellogo: fetch_logo,
            hotelType: fetch_type,
...
const addHotelLogo = async (req, res, next) => {
    try {
        ...
        if (!hotelId) 
            return res.status(404).json({status: false, msg: 'Please provide the hotel id.'});
        
        if (!name) 
            return res.status(404).json({status: false, msg: 'Please provide the name.'});
        
        if (!logo) 
            return res.status(404).json({status: false, msg: 'Please provide the logo.'});
