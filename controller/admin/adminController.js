...
const signin = async(req, res, next) => {
    try{
        ...
        if(!password) return res.status(404).json({ status: false, msg: 'Please provide a password' })
        ...
        if(password !== decyPassword) return res.status(404).json({ status: false, msg: 'email and password is not match' });
